...
public class EngineContentManager : ContentManager {
    ...
    private void Awake() {
        for (int i = 0; i < transform.childCount; i++) {
            ...
            transform.GetChild(i).GetChild(3).GetComponentInChildren<Text>().text = Mathf.Round(Engine.speed(i, Frame.weight(hangarScript.selectedFrame), Frame.size(hangarScript.selectedFrame))).ToString();
